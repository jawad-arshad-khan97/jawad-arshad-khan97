## 👋 Hi, I'm Jawad

![Profile Views](https://komarev.com/ghpvc/?username=jawad-arshad-khan97&style=flat&color=blue)
[![Follow on GitHub](https://img.shields.io/github/followers/jawad-arshad-khan97?label=Follow&style=social)](https://github.com/jawad-arshad-khan97)


Founder of SyncShire & Full-Stack Engineer with strong SDE and SDET experience.
I build scalable web platforms, compliance systems, and Test automation infra.

### 🔧 Tech Focus
- Full-Stack: Java, Springboot, React, Node, REST APIs, GraphQL
- Automation: Playwright, Selenium, API & Mobile Testing
- CI/CD & Cloud: Azure DevOps, AWS
- Performance & Security Testing
- AI-assisted development

### 🚀 Featured Work
- ZATCA-Compliant E-Invoicing Platform    - [Live Demo]([https://your-live-site.com](https://www.youtube.com/watch?v=gyzjmgTF4ew&t=1s))
- **Full-Stack CRM & E-Commerce Systems** — [Live Demo]([https://your-live-site.com](https://e-commerce-xi-ten-34.vercel.app/))
- Enterprise Test Automation Frameworks


![Tech Stack](https://skillicons.dev/icons?i=java,spring,python,postgres,mongodb,react,nodejs,selenium,docker,azure,aws,git)

Connect Me On
📫 LinkedIn: [<link>  ](https://www.linkedin.com/in/jawad-arshad-khan-23a131174/)
📧 **Email:** [jawad.khan@syncshire.com](mailto:jawad.khan@syncshire.com)

Check the products i developed on
🌐 Website: [syncshire.com](https://syncshire.com)
